...
def collect_test_labels(_data_gen_test, _data_out, _nb_classes, quick_test):
    ...
    gt_sed = np.zeros(
        (nb_batch * batch_size, _data_out[0][1], _data_out[0][2]))
    gt_doa = np.zeros(
        (nb_batch * batch_size, _data_out[0][1], _data_out[1][2]))
...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        gt = collect_test_labels(
            data_gen_val, data_out, nb_classes, params['quick_test'])
        ...
        model = keras_model.get_model(data_in=data_in, data_out=data_out, dropout_rate=params['dropout_rate'],
                                      nb_cnn2d_filt=params['nb_cnn2d_filt'], f_pool_size=params[
                                          'f_pool_size'], t_pool_size=params['t_pool_size'],
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            doa_pred = evaluation_metrics.reshape_3Dto2D(
                pred[1] if params['doa_objective'] == 'mse' else pred[1][:, :, nb_classes:])
            ...
            doa_metric[epoch_cnt, :] = evaluation_metrics.compute_doa_scores_regr_xyz(
            ...
            cls_new_metric = SELD_evaluation_metrics.SELDMetrics(
                nb_classes=data_gen_val.get_nb_classes(), doa_threshold=params['lad_doa_thresh'])
            ...
            new_seld_metric[epoch_cnt] = evaluation_metrics.early_stopping_metric(
                new_metric[epoch_cnt, :2], new_metric[epoch_cnt, 2:])

            # Visualize the metrics with respect to epochs
            plot_functions(unique_name, tr_loss, sed_metric,
                           doa_metric, seld_metric, new_metric, new_seld_metric)
            ...
            print(
                ...
                'best_seld_score: {:0.2f}, best_epoch : {}\n'.format(
                    ...
                    doa_metric[epoch_cnt, 0], doa_metric[epoch_cnt,
                                                         1]*100, seld_metric[epoch_cnt],
        ...
        print('\n\tDCASE2020 scores')
        print('\tClass-aware localization scores: DOA_error: {:0.1f}, F-score: {:0.1f}'.format(
            new_metric[best_epoch, 2], new_metric[best_epoch, 3]*100))
        print('\tLocation-aware detection scores: Error rate: {:0.2f}, F-score: {:0.1f}'.format(
            new_metric[best_epoch, 0], new_metric[best_epoch, 1]*100))

        print('\n\tDCASE2019 scores')
        print('\tLocalization-only scores: DOA_error: {:0.1f}, Frame recall: {:0.1f}'.format(
            doa_metric[best_epoch, 0], doa_metric[best_epoch, 1]*100))
        print('\tDetection-only scores: Error rate: {:0.2f}, F-score: {:0.1f}\n'.format(
            sed_metric[best_epoch, 0], sed_metric[best_epoch, 1]*100))
        ...
        model = keras_model.load_seld_model(
            '{}_model.h5'.format(unique_name), params['doa_objective'])
        ...
        test_doa_pred = evaluation_metrics.reshape_3Dto2D(
            pred_test[1] if params['doa_objective'] == 'mse' else pred_test[1][:, :, nb_classes:])

        if params['dcase_output']:
            # Dump results in DCASE output format for calculating final scores
            dcase_dump_folder = os.path.join(params['dcase_dir'], '{}_{}_{}'.format(
                task_id, params['dataset'], params['mode']))
            ...
            for file_cnt in range(test_sed_pred.shape[0]//frames_per_file):
                output_file = os.path.join(
                    dcase_dump_folder, test_filelist[file_cnt].replace('.npy', '.csv'))
                ...
                data_gen_test.write_output_format_file(
                    output_file, output_dict)

        if params['mode'] == 'dev':
            test_data_in, test_data_out = data_gen_test.get_data_sizes()
            test_gt = collect_test_labels(
                data_gen_test, test_data_out, nb_classes, params['quick_test'])
            ...
            cls_new_metric = SELD_evaluation_metrics.SELDMetrics(
                nb_classes=data_gen_test.get_nb_classes(), doa_threshold=params['lad_doa_thresh'])
            ...
            cls_new_metric.update_seld_scores_xyz(
                test_pred_blocks_dict, test_gt_blocks_dict)
            ...
            avg_scores_test.append([test_new_metric[0], test_new_metric[1],
                                   test_new_metric[2], test_new_metric[3], test_new_seld_metric])
            print('Results on test split:')

            print('\tDCASE2020 Scores')
            print('\tClass-aware localization scores: DOA Error: {:0.1f}, F-score: {:0.1f}'.format(
                test_new_metric[2], test_new_metric[3]*100))
            print('\tLocation-aware detection scores: Error rate: {:0.2f}, F-score: {:0.1f}'.format(
                test_new_metric[0], test_new_metric[1]*100))
            print('\tSELD (early stopping metric): {:0.2f}'.format(
                test_new_seld_metric))

            print('\n\tDCASE2019 Scores')
            print('\tLocalization-only scores: DOA Error: {:0.1f}, Frame recall: {:0.1f}'.format(
                test_doa_loss[0], test_doa_loss[1]*100))
            print('\tDetection-only scores:Error rate: {:0.2f}, F-score: {:0.1f}'.format(
                test_sed_loss[0], test_sed_loss[1]*100))
