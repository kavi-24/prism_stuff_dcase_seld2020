...
# INPUT DIRECTORY
# reference description directory location
ref_desc_files = '/scratch/asignal/sharath/DCASE2020_SELD_dataset/metadata_dev'
# predicted output format directory location
pred_output_format_files = 'results/2_mic_dev'
...
print('Number of predicted files: {}\nNumber of reference files: {}'.format(
    nb_pred_files, nb_ref_files))
...
for score_type in score_type_list:
    ...
    # collect files corresponding to score_type
    ...
    for split_key in np.sort(list(split_cnt_dict)):
        ...
        eval = SELD_evaluation_metrics.SELDMetrics(
            nb_classes=feat_cls.get_nb_classes(), doa_threshold=params['lad_doa_thresh'])
        for pred_cnt, pred_file in enumerate(split_cnt_dict[split_key]):
            ...
            if use_polar_format:
                pred_dict_polar = feat_cls.convert_output_format_cartesian_to_polar(
                    pred_dict)
                pred_labels = feat_cls.segment_labels(
                    pred_dict_polar, feat_cls.get_nb_frames())
            else:
                pred_labels = feat_cls.segment_labels(
                    pred_dict, feat_cls.get_nb_frames())

            # Load reference description file
            gt_dict_polar = feat_cls.load_output_format_file(
                os.path.join(ref_desc_files, pred_file.replace('.npy', '.csv')))
            if use_polar_format:
                gt_labels = feat_cls.segment_labels(
                    gt_dict_polar, feat_cls.get_nb_frames())
            else:
                gt_dict = feat_cls.convert_output_format_polar_to_cartesian(
                    gt_dict_polar)
                gt_labels = feat_cls.segment_labels(
                    gt_dict, feat_cls.get_nb_frames())
        ...
        print('\nAverage score for {} {} data using {} coordinates'.format(
            score_type, 'fold' if score_type == 'all' else split_key, 'Polar' if use_polar_format else 'Cartesian'))
        print('SELD score (early stopping metric): {:0.2f}'.format(seld_scr))
        print(
            'SED metrics: Error rate: {:0.2f}, F-score:{:0.1f}'.format(er, 100*f))
        print(
            'DOA metrics: DOA error: {:0.1f}, F-score:{:0.1f}'.format(de, 100*de_f))
